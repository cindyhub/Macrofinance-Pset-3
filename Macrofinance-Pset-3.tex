...
\[
    P_t = \frac{Y_{t} + L_t}{\rho^h + \frac{1}{\eta_t}(\rho-\rho^h)}    
\]
so we can see that increased leverage increases price. The intuition here is that with more leverage, you have more funds to put into the asset and due to there being net supply of 1 risky asset, this means the price of that asset goes up.
...
\]
so we can see that as leverage increases, volatility decreases.

In question f, the term for risk premium is:
\[
    \pi_{R, t} = \left(\frac{Y_t \sigma_Y + (1 - \rho^h) \sigma_L L_t}{\rho}\right)^2 \left(\frac{Y_{t} + L_t}{\rho^h + \frac{1}{\eta_t}(\rho-\rho^h)}\right)    
\]
so we can see that as leverage increases, risk premium increases.
\end{document}